<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    <div class="flex items-start justify-between">
      <div class="flex-1">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">{{ subscription.serviceId?.name || 'Unknown Service' }}</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">{{ subscription.serviceId?.description || 'No description available' }}</p>
      ...
      <button
        ...
        :disabled="!subscription.serviceId"
      ...
      >
        <svg v-if="loading" class="animate-spin -ml-1 mr-2 h-4 w-4 text-red-700 dark:text-red-300" fill="none" viewBox="0 0 24 24">
